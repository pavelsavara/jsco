...
import { liftFlatCoerce } from '../../src/marshal/lift';
import { lowerFlatCoerce } from '../../src/marshal/lower';
...
describe('coerceFlatLift', () => {
    test('f32→i32: reinterpret float bits as integer', () => {
        ...
        const result = liftFlatCoerce(1.0, FlatType.F32, FlatType.I32);
    ...
    test('i32→i64: widen unsigned', () => {
        const result = liftFlatCoerce(42, FlatType.I32, FlatType.I64);
        expect(result).toBe(42);
    });

    test('i32→i64: negative treated as unsigned via >>> 0', () => {
        const result = liftFlatCoerce(-1, FlatType.I32, FlatType.I64);
        expect(result).toBe(4294967295); // 0xFFFFFFFF
    });

    test('f32→i64: reinterpret f32 as i32, then widen', () => {
        const result = liftFlatCoerce(1.0, FlatType.F32, FlatType.I64);
        expect(result).toBe(1065353216); // 0x3F800000 unsigned
    });

    test('f64→i64: reinterpret f64 bits as BigInt', () => {
        const result = liftFlatCoerce(1.0, FlatType.F64, FlatType.I64);
        expect(result).toBe(4607182418800017408n); // 0x3FF0000000000000
    });

    test('same types: identity', () => {
        expect(liftFlatCoerce(42, FlatType.I32, FlatType.I32)).toBe(42);
        expect(liftFlatCoerce(3.14, FlatType.F64, FlatType.F64)).toBe(3.14);
    });
});
...
describe('coerceFlatLower', () => {
    test('i32→f32: decode integer bits as float', () => {
        ...
        const result = lowerFlatCoerce(1065353216, FlatType.I32, FlatType.F32);
        expect(result).toBeCloseTo(1.0);
    });

    test('i64→i32: wrap BigInt to 32-bit unsigned', () => {
        const result = lowerFlatCoerce(BigInt(0x1_0000_002A), FlatType.I64, FlatType.I32);
    ...
    test('i64→f32: wrap to i32 then decode as float', () => {
        ...
        const result = lowerFlatCoerce(BigInt(0x3F800000), FlatType.I64, FlatType.F32);
        expect(result).toBeCloseTo(1.0);
    });

    test('i64→f64: decode BigInt bits as double', () => {
        // 0x3FF0000000000000 = 1.0
        const result = lowerFlatCoerce(4607182418800017408n, FlatType.I64, FlatType.F64);
        expect(result).toBeCloseTo(1.0);
    });

    test('same types: identity', () => {
        expect(lowerFlatCoerce(42, FlatType.I32, FlatType.I32)).toBe(42);
        expect(lowerFlatCoerce(3.14, FlatType.F64, FlatType.F64)).toBe(3.14);
    });
});
...
describe('coerceFlatLift NaN canonicalization', () => {
    test('f32→i32: NaN → canonical 0x7fc00000', () => {
        expect(liftFlatCoerce(NaN, FlatType.F32, FlatType.I32)).toBe(0x7fc00000);
    });

    test('f32→i32: non-NaN unaffected', () => {
        // 1.0f → 0x3F800000
        expect(liftFlatCoerce(1.0, FlatType.F32, FlatType.I32)).toBe(0x3F800000);
    });

    test('f32→i64: NaN → canonical 0x7fc00000 (zero-extended)', () => {
        expect(liftFlatCoerce(NaN, FlatType.F32, FlatType.I64)).toBe(0x7fc00000);
    });

    test('f64→i64: NaN → canonical 0x7ff8000000000000n', () => {
        expect(liftFlatCoerce(NaN, FlatType.F64, FlatType.I64)).toBe(0x7ff8000000000000n);
    });

    test('f64→i64: non-NaN unaffected', () => {
        // 1.0 → 0x3FF0000000000000
        expect(liftFlatCoerce(1.0, FlatType.F64, FlatType.I64)).toBe(4607182418800017408n);
    });
});
...
describe('coerceFlatLower NaN canonicalization', () => {
    ...
    test('i32→f32: non-canonical NaN bits → JS canonical NaN value', () => {
        const result = lowerFlatCoerce(nonCanonicalNaN32_bits, FlatType.I32, FlatType.F32) as number;
    ...
    test('i32→f32: regular value unaffected', () => {
        // 0x3F800000 → 1.0
        expect(lowerFlatCoerce(0x3F800000, FlatType.I32, FlatType.F32)).toBeCloseTo(1.0);
    });

    test('i64→f32: low-32 bits non-canonical NaN → canonical NaN', () => {
        // Place sNaN bits in low 32; high 32 ignored by wrap_i64_to_i32
        const result = lowerFlatCoerce(BigInt(nonCanonicalNaN32_bits >>> 0) | 0xDEAD_BEEFn << 32n, FlatType.I64, FlatType.F32) as number;
        expect(Number.isNaN(result)).toBe(true);
        const buf = new ArrayBuffer(4);
        new Float32Array(buf)[0] = result;
        expect(new Uint32Array(buf)[0]).toBe(0x7fc00000);
    ...
    test('i64→f64: non-canonical f64 NaN bits → canonical NaN', () => {
        ...
        const result = lowerFlatCoerce(0x7ff4_0000_0000_0001n, FlatType.I64, FlatType.F64) as number;
        expect(Number.isNaN(result)).toBe(true);
    ...
    test('i64→f64: regular value unaffected', () => {
        // 0x3FF0000000000000 → 1.0
        expect(lowerFlatCoerce(4607182418800017408n, FlatType.I64, FlatType.F64)).toBeCloseTo(1.0);
    });
});
