...
/**
 * Shared helpers for integration tests through the P2-via-P3 adapter.
 * Mirrors wasip2/integration-helpers.ts but uses the adapter pipeline.
 */
...
import type { ResolutionStats } from '../../../src/resolver/api-types';
...
/** Create P2 WASI exports via the P3 adapter */
function createAdapterWasiExports(config?: {
    args?: string[];
    env?: [string, string][];
    ...
    const p3 = createWasiP3Host({
        ...
        stdout: config?.stdout
            ? new WritableStream({ write(chunk) { config.stdout!(chunk); } })
            : undefined,
        stderr: config?.stderr
            ? new WritableStream({ write(chunk) { config.stderr!(chunk); } })
            : undefined,
    });
 ...
 * Uses the P3 adapter pipeline instead of direct P2 host.
 */
export async function runConsumerScenario(
    ...
    const wasiExports = createAdapterWasiExports({
...
export function createMinimalAdapterWasiExports() {
    return createAdapterWasiExports({}) as ImportsMap;
}
