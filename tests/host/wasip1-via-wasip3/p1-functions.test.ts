...
import {
    ...
    fd_pread, fd_pwrite,
    fd_read, fd_write, fd_seek, fd_seek_async, fd_tell, fd_renumber, fd_sync, fd_readdir,
...
describe('WASI P1 clock functions', () => {
    test('clock_res_get Realtime returns 1ms (via P3 system clock)', () => {
        const { ctx, memory } = makeCtx();
        const view = getView(memory);
        const rc = clock_res_get(ctx, Clockid.Realtime, 800);
        expect(rc).toBe(Errno.Success);
        expect(view.getBigUint64(800, true)).toBe(1_000_000n);
    });

    test('clock_res_get Monotonic returns 1us (via P3 monotonic clock)', () => {
        const { ctx, memory } = makeCtx();
        const view = getView(memory);
...
describe('WASI P1 poll_oneoff', () => {
    test('handles clock subscription', async () => {
        const { ctx, memory } = makeCtx();
        const view = getView(memory);
        const inPtr = 1000;
        const outPtr = 2000;
        ...
        const rc = await poll_oneoff(ctx, inPtr, outPtr, 1, retPtr);
        expect(rc).toBe(Errno.Success);
    ...
    test('handles FdRead subscription', async () => {
        const { ctx, memory } = makeCtx();
        const view = getView(memory);
        const inPtr = 1000;
        const outPtr = 2000;
    ...
    test('handles multiple subscriptions', async () => {
        ...
        const rc = await poll_oneoff(ctx, inPtr, outPtr, 2, retPtr);
        expect(rc).toBe(Errno.Success);
        expect(view.getUint32(retPtr, true)).toBe(2);
    });

    test('unsupported clock returns Notsup in event error', async () => {
        const { ctx, memory } = makeCtx();
        const view = getView(memory);
        const inPtr = 1000;
        const outPtr = 2000;
...
describe('WASI P1 filesystem functions', () => {
    ...
    describe('fd_write to stdout', () => {
        test('writes data to stdout', async () => {
            const { ctx, memory } = makeCtx();
            const view = getView(memory);
            ...
            // nwritten at 300
            const rc = await fd_write(ctx, 1, 100, 1, 300);
            expect(rc).toBe(Errno.Success);
    ...
    describe('fd_write to stderr', () => {
        test('writes data to stderr', async () => {
            const { ctx, memory } = makeCtx();
            const view = getView(memory);
            ...
            const rc = await fd_write(ctx, 2, 100, 1, 300);
            expect(rc).toBe(Errno.Success);
        });
    });

    describe('fd_read from stdin', () => {
        test('returns 0 bytes when stdin is empty', async () => {
            const { ctx, memory } = makeCtx();
            const view = getView(memory);
            // iovec at 100: buf=200, len=10
            view.setUint32(100, 200, true);
            view.setUint32(104, 10, true);
            const rc = await fd_read(ctx, 0, 100, 1, 300);
            expect(rc).toBe(Errno.Success);
    ...
    describe('fd_seek / fd_tell', () => {
        test('seek and tell on a file fd', async () => {
            ...
            const rc = await path_open(ctx, 3, 0, 400, pathBytes.length, 0, BigInt(Rights.FdRead | Rights.FdSeek | Rights.FdTell), 0n, 0, 500);
            expect(rc).toBe(Errno.Success);
    ...
    describe('fd_filestat_get', () => {
        test('returns filestat for stdout', async () => {
            const { ctx, memory } = makeCtx();
            const view = getView(memory);
            const rc = await fd_filestat_get(ctx, 1, 500);
            expect(rc).toBe(Errno.Success);
            expect(view.getUint8(500 + FilestatLayout.filetype.offset)).toBe(Filetype.CharacterDevice);
        });
        test('returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_filestat_get(ctx, 99, 500)).toBe(Errno.Badf);
        });
    });
    ...
    describe('path_create_directory / path_remove_directory', () => {
        test('creates and removes directory', async () => {
            ...
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const createRc = await path_create_directory(ctx, 3, 400, pathBytes.length);
            expect(createRc).toBe(Errno.Success);
            const removeRc = await path_remove_directory(ctx, 3, 400, pathBytes.length);
            expect(removeRc).toBe(Errno.Success);
        });
    });

    describe('path_open / path_unlink_file', () => {
        test('opens a file and gets its stat', async () => {
            ...
            const rc = await path_open(ctx, 3, 0, 400, pathBytes.length, 0, BigInt(Rights.FdRead), 0n, 0, 500);
            expect(rc).toBe(Errno.Success);
            const fileFd = view.getUint32(500, true);
            expect(fileFd).toBeGreaterThanOrEqual(4);

            // Check filestat
            const statRc = await fd_filestat_get(ctx, fileFd, 600);
        ...
        test('path_open returns Noent for missing file', async () => {
            const { ctx, memory } = makeCtx();
        ...
        test('unlinks a file', async () => {
            ...
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const rc = await path_unlink_file(ctx, 3, 400, pathBytes.length);
            expect(rc).toBe(Errno.Success);
            // Opening should now fail
            const openRc = await path_open(ctx, 3, 0, 400, pathBytes.length, 0, BigInt(Rights.FdRead), 0n, 0, 500);
            expect(openRc).toBe(Errno.Noent);
        });
    });

    describe('path_filestat_get', () => {
        test('gets stat for existing file', async () => {
            const { ctx, memory } = makeCtx({ fs: new Map([['data.txt', 'test data']]) });
            const view = getView(memory);
            const path = 'data.txt';
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const rc = await path_filestat_get(ctx, 3, 0, 400, pathBytes.length, 500);
            expect(rc).toBe(Errno.Success);
            expect(view.getUint8(500 + FilestatLayout.filetype.offset)).toBe(Filetype.RegularFile);
        });

        test('returns Noent for missing file', async () => {
            const { ctx, memory } = makeCtx();
            const path = 'nope.txt';
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
    ...
    describe('fd_write to VFS file', () => {
        test('writes to a VFS file via fd', async () => {
            ...
            const openRc = await path_open(ctx, 3, 0, 400, pathBytes.length, 0, BigInt(Rights.FdWrite), 0n, 0, 500);
            ...
            const writeRc = await fd_write(ctx, fileFd, 600, 1, 800);
    ...
    describe('fd_pread / fd_pwrite', () => {
        test('pwrite writes at offset without changing position', async () => {
            const { ctx, memory } = makeCtx({ fs: new Map([['ptest.txt', 'abcdef']]) });
            const view = getView(memory);
            const path = 'ptest.txt';
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const rc = await path_open(ctx, 3, 0, 400, pathBytes.length, 0,
            ...
            const writeRc = await fd_pwrite(ctx, fileFd, 600, 1, 2n, 800);
            expect(writeRc).toBe(Errno.Success);
        ...
        test('pread reads at offset without changing position', async () => {
            ...
            const readRc = await fd_pread(ctx, fileFd, 600, 1, 6n, 800);
        ...
        test('pread returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_pread(ctx, 99, 0, 0, 0n, 0)).toBe(Errno.Badf);
        });

        test('pwrite returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_pwrite(ctx, 99, 0, 0, 0n, 0)).toBe(Errno.Badf);
        });
    });
    ...
    describe('fd_readdir', () => {
        test('reads directory entries', async () => {
            ...
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const rc = await path_open(ctx, 3, 0, 400, pathBytes.length, 2 /* Oflags.Directory */,
                BigInt(Rights.FdReaddir), 0n, 0, 500);
            expect(rc).toBe(Errno.Success);
            const dirFd = view.getUint32(500, true);

            // Read directory with large buffer
            const readdirRc = await fd_readdir(ctx, dirFd, 1000, 4096, 0n, 5000);
        ...
        test('returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_readdir(ctx, 99, 0, 0, 0n, 0)).toBe(Errno.Badf);
        });
    });

    describe('fd_filestat_set_size', () => {
        test('truncates a file', async () => {
            ...
            const setRc = await fd_filestat_set_size(ctx, fileFd, 5n);
            expect(setRc).toBe(Errno.Success);
        });

        test('returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_filestat_set_size(ctx, 99, 0n)).toBe(Errno.Badf);
        });
    });

    describe('fd_filestat_set_times', () => {
        test('sets times on a file', async () => {
            ...
            const setRc = await fd_filestat_set_times(ctx, fileFd, 0n, 0n, 2 | 8); // AtimNow | MtimNow
            expect(setRc).toBe(Errno.Success);
        });

        test('returns Badf for invalid fd', async () => {
            const { ctx } = makeCtx();
            expect(await fd_filestat_set_times(ctx, 99, 0n, 0n, 0)).toBe(Errno.Badf);
        });
    });

    describe('fd_read from VFS file', () => {
        test('reads data from a VFS file', async () => {
            ...
            const readRc = await fd_read(ctx, fileFd, 600, 1, 800);
            expect(readRc).toBe(Errno.Success);
    ...
    describe('fd_seek whence variants', () => {
        test('seek Cur from current position', async () => {
        ...
        test('seek End from end of file', async () => {
            ...
            const seekRc = await fd_seek_async(ctx, fileFd, 0n, Whence.End, 600);
            expect(seekRc).toBe(Errno.Success);
    ...
    describe('path_filestat_set_times', () => {
        test('sets times on a path', async () => {
            const { ctx, memory } = makeCtx({ fs: new Map([['timed.txt', 'data']]) });
            const path = 'timed.txt';
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const setRc = await path_filestat_set_times(ctx, 3, 0, 400, pathBytes.length, 0n, 0n, 2 | 8);
            expect(setRc).toBe(Errno.Success);
        });

        test('returns Noent for missing path', async () => {
            const { ctx, memory } = makeCtx();
            const path = 'missing.txt';
            const pathBytes = new TextEncoder().encode(path);
            new Uint8Array(memory.buffer, 400, pathBytes.length).set(pathBytes);
            const setRc = await path_filestat_set_times(ctx, 3, 0, 400, pathBytes.length, 0n, 0n, 0);
            expect(setRc).toBe(Errno.Noent);
        });
    });

    describe('path_link', () => {
        test('links a file', async () => {
            ...
            const rc = await path_link(ctx, 3, 0, 400, oldBytes.length, 3, 500, newBytes.length);
            expect(rc).toBe(Errno.Success);
        });
    });
    ...
    describe('path_rename', () => {
        test('renames a file', async () => {
            ...
            const newBytes = new TextEncoder().encode(newp);
            new Uint8Array(memory.buffer, 500, newBytes.length).set(newBytes);
            const rc = await path_rename(ctx, 3, 400, oldBytes.length, 3, 500, newBytes.length);
            expect(rc).toBe(Errno.Success);
        });
    });

    describe('path_symlink', () => {
        test('creates a symlink', async () => {
            ...
            const rc = await path_symlink(ctx, 400, targetBytes.length, 3, 500, linkBytes.length);
            expect(rc).toBe(Errno.Success);
        });
    });

    describe('path_readlink', () => {
        test('reads a symlink', async () => {
            const { ctx, memory } = makeCtx({ fs: new Map([['target.txt', 'data']]) });
            const view = getView(memory);
            ...
            const linkBytes = new TextEncoder().encode(link);
            new Uint8Array(memory.buffer, 500, linkBytes.length).set(linkBytes);
            await path_symlink(ctx, 400, targetBytes.length, 3, 500, linkBytes.length);

            // Read it back
            const rc = await path_readlink(ctx, 3, 500, linkBytes.length, 700, 256, 800);
            expect(rc).toBe(Errno.Success);
