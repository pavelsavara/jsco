...
>   - **G3 forwarder body streaming:** 256 KiB POST body round-trips intact via `wit_stream` chunked forwarding through the post-return spawned forwarder.
>   - **G4 mixed paths on one instance:** N=8 interleaved fast-path + forwarder requests run concurrently on the same instance.
>   - **G5 instance lifecycle:** 3 sequential rounds of K=4 parallel requests on one instance verifies handle/waitable/subtask tables drain cleanly between rounds.
>   - Verified by `tests/host/wasip3/node/http-reactor-concurrent.test.ts`.
>   - Fixes that landed: (a) Phase 0 SubResource pre-scan in `registerInstanceLocalTypes` so `Eq(N)` aliases inherit canonical resource IDs (was producing `21 != 25` mismatches on `headers = fields`); (b) `task.return` flat-vs-spilled threshold corrected to `MAX_FLAT_PARAMS=16` (was incorrectly using `MAX_FLAT_RESULTS=1` and reading garbage from `ptr=0`); (c) `streams.removeReadable` returns a buffer-backed iterable when the original `AsyncIterable` was already pumped (response body was an empty already-drained iterator); (d) P2-via-P3 server adapter now wraps responses in `{tag:'ok', val:...}` to match the spec-strict serve() contract; (e) `createAsyncLiftWrapper` now resolves the handler promise as soon as `task.return` is invoked and continues driving the event loop (post-return spawned subtasks) in the background — without this, a `wit_bindgen::spawn` forwarder writing > 64 KiB into a stream pipe deadlocks because the host is stuck in `await handler.handle()` and cannot drain the read side concurrently.
> - **OPEN — Group 2: yield-from-cancel-only.** Decision pending: default-on when JSPI is enabled vs. opt-in `RuntimeConfig` flag.
> - **JSPI + Parallel scenarios P1, P2, P9-lite, P10, P11 LANDED.** `tests/host/wasip3/jspi-parallel.test.ts` — uses `multi-async-p3-wat` with deferred-Promise host imports (jsco auto-wraps in `WebAssembly.Suspending`) plus formal acceptance criteria (`unhandledRejection`/`uncaughtException` capture + `setImmediate` watchdog liveness counter). Covers P1 (sequential queue while suspended), P2 (N=8 parallel JSPI suspends, out-of-order resolve), P2b (two-subtask waitable-set with out-of-order JSPI resolve), P9-lite (two N=32 bursts on one instance verifying clean drain between rounds), P10 (3-round staggered re-invocation on one instance), and P11 (rejected void-return host call — guest sees normal subtask completion, instance stays usable for further calls).
> - **Fix landed: rejected host-import Promise no longer poisons the instance.** `src/marshal/trampoline-lower.ts` `handleLowerResult` / `handleLowerResultSpilled` previously called `ctx.abort()` on Promise rejection, permanently poisoning the whole component instance. Now the rejection is rethrown without abort: async-lower path consumes it cleanly via `subtask-table` (state → RETURNED), and sync-lower JSPI path delivers a normal wasm trap to the suspended caller. The instance-poison case in `tests/runtime/abort-dispose-integration.test.ts` was flipped to assert the new "instance survives" semantics.
> - **OPEN — Remaining JSPI scenarios.** P3, P4, P5, P6, P7, P8, P12 still need fixtures or stub host imports beyond the simple `slow-fn` shape (e.g. spawn-from-handler, cancellation across suspend, backpressure interaction, borrow-handle survival).
