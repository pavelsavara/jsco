...
> - **JSPI + Parallel scenarios P1, P2, P10 LANDED.** `tests/host/wasip3/jspi-parallel.test.ts` — uses `multi-async-p3-wat` with deferred-Promise host imports (jsco auto-wraps in `WebAssembly.Suspending`) plus formal acceptance criteria (`unhandledRejection`/`uncaughtException` capture + `setImmediate` watchdog liveness counter). Covers P1 (sequential queue while suspended), P2 (N=8 parallel JSPI suspends, out-of-order resolve), P10 (3-round staggered re-invocation on one instance), and P2b (two-subtask waitable-set with out-of-order JSPI resolve).
> - **OPEN — Remaining JSPI scenarios.** P3, P4, P5, P6, P7, P8, P9, P11, P12 still need fixtures or stub host imports beyond the simple `slow-fn` shape (e.g. cancellation across suspend, backpressure interaction, borrow-handle survival).
